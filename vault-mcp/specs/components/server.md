...
- `src/server.py` — entrypoint. Creates the FastAPI app, registers tables, initializes parsers (which seeds the database via immediate watcher firings), and starts the watcher.
...
3. Construct the database and watcher (see `components/database.md`, `components/asyncfile.md`).
...
6. **Initialize parsers.** For each system, instantiate its parser and call `parser.initialize(database, watcher)`. This registers the system's watchers; the watchers fire immediately for current matching state, which seeds the database transitively via `parse(...)` → `database.update(...)`. There is no separate scan / seed pass.
7. Walk `src/routes/<system>/<op>/route.py` modules and register each handler.
8. Start the watcher's live event loop (drives both inbound parsing and outbound flush dispatch — see `components/asyncfile.md`).
...
- Tables are registered and the FastAPI app, MCP wrapper, and routes are all built — but parsers are not initialized (so the database is empty) and the watcher loop is not started.
- Vault-dependent endpoints respond with HTTP 503 until parser initialization completes.
- A background retry loop re-probes Obsidian on a fixed interval; once the vault becomes reachable, the deferred parser-initialization step runs (seeding the database via immediate watcher firings) and the watcher loop starts, transitioning the server out of degraded mode without a restart.
...
On shutdown, the watcher loop is cancelled, any dirty taskfiles are flushed by invoking each system's flush callback directly, and the database is closed.
